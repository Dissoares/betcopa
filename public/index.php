...
try {
    if (str_starts_with($uri, '/api/')) {
        ...
        routePattern('/^\/api\/admin\/jogos\/(\d+)$/', 'POST',
            fn(int $id) => $gameCtrl->update($id));
        routePattern('/^\/api\/admin\/jogos\/(\d+)\/excluir$/', 'POST',
        ...
        routePattern('/^\/api\/admin\/usuarios\/(\d+)\/promover$/',    'POST', fn(int $id) => $adminCtrl->promoteUser($id));
        routePattern('/^\/api\/admin\/usuarios\/(\d+)\/rebaixar$/',    'POST', fn(int $id) => $adminCtrl->demoteUser($id));
