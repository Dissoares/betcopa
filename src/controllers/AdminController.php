...
class AdminController
{
    ...
    public function promoteUser(int $id): void
    {
        ...
        Logger::info('Usuário promovido a admin', ['id' => $id]);
        jsonResponse(['message' => 'Usuário promovido a administrador.']);
    }

    public function demoteUser(int $id): void
    {
        Csrf::verify();
        ensureAdmin($this->adminEmail);
        $this->users->setAdmin($id, false);
        Logger::info('Usuário rebaixado de admin', ['id' => $id]);
        jsonResponse(['message' => 'Permissão de admin removida.']);
    }
