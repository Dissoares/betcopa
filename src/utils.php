...
function ensureAdmin(string $adminEmail = ''): void
{
    ...
    // Suporta múltiplos admins: verifica flag is_admin na sessão
    // Mantém retrocompatibilidade com admin_email para instalações antigas
    $isAdminSession = !empty($_SESSION['is_admin']);
    $isAdminEmail   = $adminEmail && !empty($_SESSION['user_email']) && $_SESSION['user_email'] === $adminEmail;
    if (!$isAdminSession && !$isAdminEmail) {
